...
//        int selectedCount = androidImagePicker.getSelectImageCount() ;
        ...
        onImageSelectChange(0, null, 0, androidImagePicker.getSelectLimit());
    }
...
//    }
    ...
    public void onImageSelectChange(int position, ImageItem item, int selectedItemsCount, int maxSelectLimit) {
        ...
        if(selectedItemsCount == 0 && androidImagePicker.getChooseNum()  == 0){
            mBtnOk.setText(getResources().getString(R.string.complete));
        }else{
            int num = selectedItemsCount + androidImagePicker.getChooseNum() ;
            mBtnOk.setEnabled(false);
            mBtnOk.setEnabled(true);
            mBtnOk.setText("完成("+ num +"/"+ androidImagePicker.getMaxSelect() +")");
